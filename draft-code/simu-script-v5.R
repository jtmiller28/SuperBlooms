### A version of the simulation where we use additive probablity laws rather than mutliplication (non-independent events  ### Making superblooms site specific 
  ...
  gen.background <- function(sample.year.level = c("High", "Medium", "Low")){
    ...
    background_data <- fread("/home/jt-miller/Gurlab/SuperBlooms/data/processed/test50km-background-site-data.csv")
  ...
  sim.superbloom <- function(
    ...
    # Backgrounds
    gen.background <- function(sample.year.level = c("High", "Medium", "Low")){
      background_data <- data.table::fread("/home/jt-miller/Gurlab/SuperBlooms/data/processed/test50km-background-site-data.csv")
      if(sample.year.level == "High") {
    ...
    sim_obs <- purrr::map_dfr(1:n_years, function(y) {
      ...
      site_results <- purrr::map_dfr(1:n_cells, function(i) {
        ...
        # replacing the following with an additive probablistic approach
        ...
        total_logit <- logit_day + logit_spat + logit_year
        total_pdet <- plogit(total_logit)
